...
void IniciaTiva() {
	...
	****************/
	...
	ui32periodo = (SysCtlClockGet() / 1) / 2;
	TimerLoadSet(TIMER0_BASE, TIMER_A, ui32periodo -1);
...
void main(void)
{
    ...
    *modo_atual = START;
	...
	/*valor_anual->end.word = ANO1 + 1;
    valor_diaria->end.word = MES1 + 3;
    EEPROM_PegaMedia(valor_anual);
    EEPROM_PegaLeitura(valor_diaria);*/
	...
	while(1) {
		...
		if(*modo_atual == START)
			...
			*modo_atual = START;
			...
			valor_diaria->end.word = referencia.end_diaria.word;
			EEPROM_PegaMedia(valor_anual);
			EEPROM_PegaLeitura(valor_diaria);*/
