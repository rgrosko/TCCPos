...
int main(void) {
	...
	***********************/
	...
	GPIOPinWrite(GPIO_PORTF_BASE, GPIO_PIN_1, GPIO_PIN_1); 				  //LED START: RED
	GPIOPinWrite(GPIO_PORTD_BASE, GPIO_PIN_3, 0X00);
	while(1) {
		if(GPIOPinRead(GPIO_PORTD_BASE, GPIO_PIN_1)) {
			GPIOPinWrite(GPIO_PORTF_BASE, GPIO_PIN_3, GPIO_PIN_3);		  //LED SIGNAL: YELLOW
			...
			if(flag == 0x00) {											  //START READ CONDITION - F0X00
				...
				GPIOPinWrite(GPIO_PORTF_BASE, GPIO_PIN_2, GPIO_PIN_2);    //LED START TX: WHITE
				...
				GPIOPinWrite(GPIO_PORTF_BASE, GPIO_PIN_2, 0x00); //LED END TX: WHITE
				TimerDisable(TIMER0_BASE, TIMER_A);
				flag = 0x00;
				SysCtlDelay(10000000);
			}
		} else {
			GPIOPinWrite(GPIO_PORTF_BASE, GPIO_PIN_3, 0x00);			  //LED NO SIGNAL: RED
...
void GPIODIntHandler(void) {
	...
	if(GPIOPinRead(GPIO_PORTD_BASE, GPIO_PIN_1))  ui32Pulso++;
}
